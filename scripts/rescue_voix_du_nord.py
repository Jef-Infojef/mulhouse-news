...
def main():
    print("[*] Sauvetage des articles La Voix du Nord...")
    ...
    cur.execute("""
        ...
        FROM \"Article\" 
        WHERE (source LIKE '%Voix du Nord%')
    ...
    total = len(articles)
    print(f"[*] {total} articles La Voix du Nord à traiter.")
    ...
    cur.close()
    conn.close()
    print(f"\n[*] Terminé. {repaired} articles La Voix du Nord sauvés.")
