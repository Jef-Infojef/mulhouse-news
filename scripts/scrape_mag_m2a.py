"""Scraper dédié mag.mulhouse-alsace.fr (m2A le mag) — ingestion via sitemap."""
import argparse
import json
import os
...
SOURCE = "m2A le mag"
SITEMAP_INDEX = "https://mag.mulhouse-alsace.fr/sitemap_index.xml"
...
def is_article_url(url: str) -> bool:
    if "mag.mulhouse-alsace.fr" not in url:
        return False
    if any(part in url for part in SKIP_PARTS):
        return False
    path = url.replace("https://mag.mulhouse-alsace.fr/", "").strip("/")
    if not path or "/" in path:
        return False
    if path.lower() in SKIP_SLUGS:
        return False
    return True
...
def backfill_missing_content(cur, conn, limit: int | None, dry_run: bool) -> dict:
    ...
    print(f"[*] {len(rows)} articles m2A le mag sans contenu à rattraper.")
    for i, (article_id, title, link) in enumerate(rows, 1):
        ...
        soup = fetch_mag_m2a_page(link)
        if not soup:
            stats["errors"] += 1
            continue

        parsed = parse_mag_m2a_article(soup, link)
...
def seed_missing_articles(cur, conn, days: int | None, limit: int | None, dry_run: bool) -> dict:
    ...
    for i, entry in enumerate(missing, 1):
        ...
        soup = fetch_mag_m2a_page(url)
        if not soup:
            stats["errors"] += 1
            continue

        parsed = parse_mag_m2a_article(soup, url)
...
def main():
    parser = argparse.ArgumentParser(description="Scraper dédié mag.mulhouse-alsace.fr (m2A le mag)")
    parser.add_argument("--days", type=int, default=0, help="Limiter aux N derniers jours (0 = tout)")
    parser.add_argument("--limit", type=int, default=0, help="Nombre max d'articles")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--backfill-content", action="store_true")
