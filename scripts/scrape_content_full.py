...
def run_image_scripts():
    ...
    try:
        ...
        subprocess.run(["npx", "tsx", "--tsconfig", "tsconfig.scripts.json", "scripts/download_images.ts"], check=True)
        subprocess.run(["npx", "tsx", "--tsconfig", "tsconfig.scripts.json", "scripts/sync_to_b2.ts"], check=True)
        return "Success"
    except Exception as e:
