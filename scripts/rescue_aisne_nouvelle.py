...
def main():
    print("[*] Sauvetage des articles L'Aisne nouvelle...")
    ...
    cur.execute("""
        ...
        FROM \"Article\" 
        WHERE (source LIKE '%Aisne nouvelle%')
    ...
    total = len(articles)
    print(f"[*] {total} articles L'Aisne nouvelle à traiter.")
    ...
    cur.close()
    conn.close()
    print(f"\n[*] Terminé. {repaired} articles L'Aisne nouvelle sauvés.")
